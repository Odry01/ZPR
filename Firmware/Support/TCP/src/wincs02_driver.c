...
SYS_WINCS_RESULT_t WINCS02_DRIVER_WIFI_Callback(SYS_WINCS_WIFI_EVENT_t event, SYS_WINCS_WIFI_HANDLE_t wifiHandle)
{
    SYS_CONSOLE_PRINT("WIFI EVENT FIRED: event=%d\r\n", event);
    switch (event)
    {
...
SYS_WINCS_RESULT_t WINCS02_DRIVER_SOCKET_Callback(uint32_t socket, SYS_WINCS_NET_SOCK_EVENT_t event, SYS_WINCS_NET_HANDLE_t netHandle)
{
    SYS_CONSOLE_PRINT("SOCKET EVENT FIRED: event=%d socket=%lu\r\n", event, socket);
    switch (event)
    {
        case SYS_WINCS_NET_SOCK_EVENT_CONNECTED:
        {
            SYS_CONSOLE_MESSAGE("SYS_WINCS_NET_SOCK_EVENT_CONNECTED\r\n");
            SOCKET_CONFIG.sockID = socket;
        ...
        {
            SYS_CONSOLE_MESSAGE("SYS_WINCS_NET_SOCK_EVENT_DISCONNECTED\r\n");
            wincs02_driverData.TCP_CONNECT_STATUS = false;
            break;
        }
        ...
        {
            SYS_CONSOLE_MESSAGE("SYS_WINCS_NET_SOCK_EVENT_SEND_COMPLETE\r\n");
        ...
        {
            SYS_CONSOLE_MESSAGE("SYS_WINCS_NET_SOCK_EVENT_ERROR\r\n");
            wincs02_driverData.TCP_CONNECT_STATUS = false;
            break;
        }
...
void WINCS02_DRIVER_NET_Config(void)
{
    ...
    SOCKET_CONFIG.sockID = 4294967295;
}
...
void WINCS02_DRIVER_Tasks(void)
{
    switch (wincs02_driverData.state)
    {
        ...
        {
            if (SYS_WINCS_NET_SockSrvCtrl(SYS_WINCS_NET_SOCK_SET_SRVC_CALLBACK, WINCS02_DRIVER_SOCKET_Callback) == SYS_WINCS_PASS)
            {
        ...
        case WINCS02_DRIVER_STATE_WAIT_FOR_SEND_MESSAGE_PAYLOAD:
        {
            ...
            {
                wincs02_driverData.DATA_TRANSFER_COMPLETE_STATUS = false;
